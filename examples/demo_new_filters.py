...
async def main():
    ...
    crawler_css = BaseCrawler(
        content_filters=[css_filter],
        headless=True
    )
    ...
    crawler_xpath = BaseCrawler(
        content_filters=[xpath_filter],
        headless=True
    )
    ...
    crawler_regex = BaseCrawler(
        content_filters=[regex_filter],
        headless=True
    )
    ...
    crawler_regex_replace = BaseCrawler(
        content_filters=[regex_replace_filter],
        headless=True
    )
    
    result_regex_replace = await crawler_regex_replace.crawl(url)
    print("Product title with replacement:")
    if result_regex_replace["success"] and result_regex_replace.get("extracted_content"):
    ...
    combined_crawler = BaseCrawler(
        content_filters=[title_filter, price_filter, image_filter],
        headless=True
    )
