...
def display_config_summary(config: AppConfig) -> None:
    ...
    print(f"  - Rate Limit: {config.crawler.rate_limit.requests} requests per {config.crawler.rate_limit.period} seconds")
    ...
    if config.llm.api_key:
        print(f"  - API Key: {'*' * 8}{config.llm.api_key[-4:] if len(config.llm.api_key) > 4 else ''}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate crawl4ai_llm configuration file")
    ...
    example_parser.add_argument(
        "output_file",
        help="Path to save example configuration file"
    )
    example_parser.add_argument(
        "--format",
