...
def format_product_summary(product: ProductData) -> Dict[str, Any]:
    ...
    return {
        ...
        "price": f"{product.price.amount} {product.price.currency}" if product.price else "N/A",
        "brand": product.brand or "N/A",
        "available": "Yes" if product.available else "No",
        "images": len(product.images) if product.images else 0,
        "date_extracted": product.date_extracted.strftime("%Y-%m-%d %H:%M:%S") if product.date_extracted else "N/A"
    }
...
def search_products(
            ...
            if format_type == "json":
                ...
                if product.variants:
                    ...
                    for i, variant in enumerate(product.variants[:5]):
                        price_str = f"{variant.price.amount} {variant.price.currency}" if variant.price else "N/A"
    ...
    try:
        ...
        if format_type == "json":
            ...
            # Table output
            headers = ["ID", "Title", "Price", "Brand", "Available", "Images", "Date Extracted"]
            rows = []
            ...
            for i, product in enumerate(products):
                summary = format_product_summary(product)
                print(f"{i+1}. [{summary['id']}] {summary['title']} - {summary['price']}")
            print(f"\nTotal: {len(products)} product(s)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Search and display products from storage")
    parser.add_argument(
        "--storage", 
        default="./data",
    ...
    parser.add_argument(
        "--query",
        help="Text query to filter products"
    )
    parser.add_argument(
        "--sort",
    ...
    parser.add_argument(
        "--available",
        action="store_true",
        help="Only show available products"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--id",
        help="Display details for a specific product ID"
    )
