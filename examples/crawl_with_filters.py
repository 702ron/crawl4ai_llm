...
async def crawl_with_filters():
    ...
    bm25_filter = ContentFilter(
        filter_type="bm25", query="product details specifications price", threshold=0.5
    )

    crawler = BaseCrawler(cache_mode=CacheMode.ENABLED, content_filters=[bm25_filter])
    ...
    crawler = BaseCrawler(
        cache_mode=CacheMode.ENABLED, content_filters=[pruning_filter]
    )
    ...
    try:
        ...
        crawler = BaseCrawler(
            cache_mode=CacheMode.ENABLED, content_filters=[llm_filter]
        )
