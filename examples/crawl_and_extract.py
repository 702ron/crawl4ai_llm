...
from crawl4ai_llm.extraction.product_extractor import (ExtractionStrategy,
                                                       ProductExtractor)
...
async def crawl_and_extract(url, extraction_strategy, storage_path, use_fallback=False):
    ...
    config = Config(
        ...
        storage=StorageConfig(type="json", path=storage_path, use_uuid=True),
    )
    ...
    try:
        ...
        product_data = await extractor.extract(
            url, content, strategy, fallback=use_fallback
        )
        ...
        logger.info(
            f"Extraction completed successfully in {execution_time:.2f} seconds"
        )
        ...
        summary = {
            ...
            "price": (
                f"{product_data.price.amount} {product_data.price.currency}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Crawl and extract product data from e-commerce websites"
    )
