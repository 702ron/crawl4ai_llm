...
from ..storage import (DuplicateProductError, ProductNotFoundError,
                       StorageError, get_storage)
from .auth import User, UserRole, check_rate_limit, get_current_user, has_role
...
async def get_configuration(
    user: User = Depends(has_role([UserRole.ADMIN]))
):
...
async def list_products(
    ...
    sort_by: Optional[str] = Query(
        None, description="Field to sort by (title, brand, etc.)"
    ),
...
async def delete_product(
    product_id: str, 
    user: User = Depends(has_role([UserRole.ADMIN, UserRole.USER])),
    storage=Depends(get_storage_instance)
):
