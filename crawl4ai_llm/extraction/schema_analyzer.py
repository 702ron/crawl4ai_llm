...
from .schema_validator import (COMMON_PRODUCT_FIELDS, REQUIRED_FIELDS,
                               SchemaValidator)
...
class SchemaAnalyzer:
    ...
    def _extract_domain_hints(self, url: str) -> Dict[str, Any]:
        ...
        if any(
            kw in domain for kw in ["amazon", "ebay", "walmart", "target", "bestbuy"]
        ):
    ...
    def _generate_field_candidates(
        self, soup: BeautifulSoup, domain_hints: Dict[str, Any]
    ...
    def _find_candidates_for_field(
        self, field_name: str, soup: BeautifulSoup, domain_hints: Dict[str, Any]
    ) -> List[ElementScore]:
        """
        ...
        for selector_info in patterns.get("selectors", []):
            ...
            try:
                ...
                logger.warning(
                    f"Error finding elements for selector {selector}: {str(e)}"
                )

        # Find elements using heuristic search
        heuristic_candidates = self._find_candidates_heuristically(
            soup, field_name, patterns
        )
    ...
    def _get_field_patterns(
        self, field_name: str, domain_hints: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
            ...
            domain_patterns = {
                "sizes": {
                    "selectors": [
                        ...
                        {
                            "selector": "[data-testid='size-options']",
                            "attribute": "text",
                            "base_score": 0.9,
                        },
                    ],
                ...
                "colors": {
                    "selectors": [
                        ...
                        {
                            "selector": "[data-testid='color-options']",
                            "attribute": "text",
                            "base_score": 0.9,
                        },
                    ],
    ...
    def _find_candidates_heuristically(
        self, soup: BeautifulSoup, field_name: str, patterns: Dict[str, Any]
    ) -> List[ElementScore]:
        """
        ...
        for keyword in keywords:
            ...
            for element in elements:
                ...
                content = element.get_text(strip=True)
                score = self._calculate_element_score(
                    element, field_name, "text", content, 0.3
                )
        ...
        if regex_pattern:
            ...
            for element in elements:
                ...
                content = element.get_text(strip=True)
                score = self._calculate_element_score(
                    element, field_name, "text", content, 0.4
                )
        ...
        for class_name in common_classes:
            ...
            for element in elements:
                ...
                score = self._calculate_element_score(
                    element, field_name, "text", content, 0.5
                )
        ...
        if field_name == "images":
            ...
            product_containers = soup.find_all(
                ["div", "section"],
                class_=re.compile(r"product|item|gallery", re.IGNORECASE),
            )
    ...
    def _calculate_element_score(
        ...
        if field_name.lower() in element_id:
            score += 0.2
        if any(
            variant in element_id for variant in self._get_field_variants(field_name)
        ):
            score += 0.1
        ...
        if field_name.lower() in element_classes:
            score += 0.15
        if any(
            variant in element_classes
            for variant in self._get_field_variants(field_name)
        ):
            score += 0.1
        ...
        if field_name == "price" and re.search(r"(\$|€|£|\d+[,.]\d{2})", content):
            ...
            field_name == "images"
            and attribute == "src"
            and re.search(r"\.(jpg|jpeg|png|webp)", content)
        ):
            score += 0.2
    ...
    def _create_schema_from_candidates(
        self, candidates: Dict[str, List[ElementScore]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        schema = {"name": "Generated Schema", "fields": []}
        ...
        for field_name, field_candidates in candidates.items():
            ...
            if top_candidate.selector in selected_selectors:
                ...
                for candidate in field_candidates[1:]:
                    if (
                        candidate.selector not in selected_selectors
                        and candidate.score > 0.4
                    ):
