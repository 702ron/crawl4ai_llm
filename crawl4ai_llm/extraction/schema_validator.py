...
class SchemaValidator:
    ...
    def validate_schema(self, schema: Any) -> bool:
        ...
        if not isinstance(schema, dict):
            self.errors.append(f"Schema must be a dictionary, got {type(schema).__name__}")
            if self.strict:
    ...
    def _validate_direct_fields_schema(self, schema: Dict[str, Any]) -> bool:
        ...
        for name in field_names:
            ...
            if not isinstance(field_value, dict):
                self.errors.append(f"Field '{name}' must be a dictionary, got {type(field_value).__name__}")
                valid_fields = False
                continue
    ...
    def _validate_schema_structure(self, schema: Dict[str, Any]) -> bool:
        ...
        if not isinstance(schema["fields"], list):
            self.errors.append(f"'fields' must be a list, got {type(schema['fields']).__name__}")
            return False
        ...
        field_names = {field.get("name") for field in schema["fields"] if isinstance(field, dict) and "name" in field}
        required_fields = {"title", "price"}
        ...
        if duplicate_fields:
            self.errors.append(f"Duplicate field names found: {', '.join(duplicate_fields)}")
    ...
    def _validate_field_dict(self, field: Dict[str, Any]) -> bool:
        ...
        if not self.validate_selector(field.get("selector")):
            self.errors.append(f"Field '{name}' has invalid CSS selector: {field.get('selector')}")
            return False

        # Validate attribute
        if "attribute" in field and not isinstance(field["attribute"], str):
            self.errors.append(f"Field '{name}' has invalid attribute: must be a string")
            return False

        # Validate array flag
        if "array" in field and not isinstance(field["array"], bool):
            self.errors.append(f"Field '{name}' has invalid 'array' value: must be a boolean")
            return False

        # Validate required flag
        if "required" in field and not isinstance(field["required"], bool):
            self.errors.append(f"Field '{name}' has invalid 'required' value: must be a boolean")
            return False
    ...
    def _validate_price_parsing(self, config: Any, field_name: str) -> bool:
        ...
        if not isinstance(config, dict):
            self.errors.append(f"Field '{field_name}' has invalid price_parsing: must be a dictionary")
            return False

        # Validate currency_symbols
        if "currency_symbols" in config and not isinstance(config["currency_symbols"], list):
            self.errors.append(f"Field '{field_name}' has invalid price_parsing.currency_symbols: must be a list")
            valid = False

        # Validate decimal_separator
        if "decimal_separator" in config and not isinstance(config["decimal_separator"], str):
            self.errors.append(f"Field '{field_name}' has invalid price_parsing.decimal_separator: must be a string")
            valid = False

        # Validate thousands_separator
        if "thousands_separator" in config and not isinstance(config["thousands_separator"], str):
            self.errors.append(f"Field '{field_name}' has invalid price_parsing.thousands_separator: must be a string")
            valid = False

        # Validate strip_non_numeric
        if "strip_non_numeric" in config and not isinstance(config["strip_non_numeric"], bool):
            self.errors.append(f"Field '{field_name}' has invalid price_parsing.strip_non_numeric: must be a boolean")
            valid = False
    ...
    def _correct_direct_fields_schema(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for name in field_names:
            ...
            if isinstance(field_value, str):
                ...
                if name == "price":
                    field_dict["price_parsing"] = COMMON_PRODUCT_ATTRIBUTES["price"]["price_parsing"]
            ...
            # Handle dictionary values
                ...
                if "selector" not in field_dict or not self.validate_selector(field_dict["selector"]):
                    if name in COMMON_PRODUCT_ATTRIBUTES:
                        field_dict["selector"] = COMMON_PRODUCT_ATTRIBUTES[name]["selector"]
                    else:
    ...
    def _correct_fields_list_schema(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in corrected_schema["fields"]:
            ...
            if "selector" not in field or not self.validate_selector(field["selector"]):
                ...
                if field["name"] in COMMON_PRODUCT_ATTRIBUTES:
                    field["selector"] = COMMON_PRODUCT_ATTRIBUTES[field["name"]]["selector"]
                else:
            ...
            if field["name"] == "price" and ("price_parsing" not in field or not isinstance(field["price_parsing"], dict)):
                field["price_parsing"] = COMMON_PRODUCT_ATTRIBUTES["price"]["price_parsing"]
                ...
                if "currency_symbols" not in price_parsing or not isinstance(price_parsing["currency_symbols"], list):
                    price_parsing["currency_symbols"] = ["$", "€", "£", "¥"]
                    
                if "decimal_separator" not in price_parsing or not isinstance(price_parsing["decimal_separator"], str):
                    price_parsing["decimal_separator"] = "."
                    
                if "thousands_separator" not in price_parsing or not isinstance(price_parsing["thousands_separator"], str):
                    price_parsing["thousands_separator"] = ","
                    
                if "strip_non_numeric" not in price_parsing or not isinstance(price_parsing["strip_non_numeric"], bool):
                    price_parsing["strip_non_numeric"] = True
            
            # Correct array flag for image fields
            if field["name"] == "images" and ("array" not in field or not isinstance(field["array"], bool)):
                field["array"] = True
        ...
        if "name" not in corrected_schema or not isinstance(corrected_schema["name"], str):
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        if isinstance(self.schema, dict):
            ...
            if "fields" in self.schema and isinstance(self.schema["fields"], list):
                ...
                special_keys = ["name", "description"]
                field_names = [key for key in self.schema.keys() if key not in special_keys]
                fields = []
                for name in field_names:
                    field_dict = {"name": name}
                    if name in ["title", "price"]:
            ...
            required_fields = [f for f in fields if isinstance(f, dict) and f.get("required") is True]
            ...
            schema_fields = {f.get("name") for f in fields if isinstance(f, dict) and "name" in f}
    ...
    def suggest_improvements(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in fields:
            ...
            if field_name == "images" and ("array" not in field or not field.get("array")):
                field_suggestions.append(
            ...
            if "attribute" not in field:
                if field_name == "images":
                    field_suggestions.append("Add 'attribute: \"src\"' to extract image URLs.")
                else:
                    field_suggestions.append("Add 'attribute: \"text\"' to extract text content.")
