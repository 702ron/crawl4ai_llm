...
class SchemaMerger:
    ...
    def merge(self, schemas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        valid_schemas = [
            schema
            for schema, score in scored_schemas
        ...
        merged_schema: Dict[str, Any] = {"fields": []}
        ...
        for schema in valid_schemas:
            for field_data in schema.get("fields", []):
                ...
                if field.name in field_map:
                    ...
                    existing_field = SchemaField.from_dict(
                        merged_schema["fields"][existing_idx]
                    )
                    ...
                    if hasattr(field, "attribute") and hasattr(existing_field, "attribute"):
                        if self._is_better_attribute(
                            field.attribute or "", existing_field.attribute or ""
                        ):
                    ...
                    if hasattr(field, "description") and hasattr(existing_field, "description"):
                        field_description = getattr(field, "description", None)
                        existing_description = getattr(existing_field, "description", None)
                        
                        if field_description and existing_description:
                            if len(str(field_description)) > len(str(existing_description)):
                                merged_schema["fields"][existing_idx]["description"] = field_description
        ...
        is_valid = self.validator.validate(merged_schema)
        if not is_valid:
    ...
    def _compare_selectors(self, new_selector: str, existing_selector: str) -> bool:
        ...
        if new_specificity == existing_specificity and len(new_selector) < len(
            existing_selector
        ):
            return True
...
def merge_schemas(
    schemas: List[Dict[str, Any]], quality_threshold: float = 0.6
) -> Dict[str, Any]:
    """
...
def enhance_schema_with_feedback(
    schema: Dict[str, Any], successful_fields: List[str], failed_fields: List[str]
) -> Dict[str, Any]:
    """
...
def optimize_schema_for_site(
    schema: Dict[str, Any], site_url: str, extraction_success_rate: float
) -> Dict[str, Any]:
    """
