...
from .schema_validator import COMMON_PRODUCT_FIELDS, SchemaValidator
...
class SchemaEnhancer:
    ...
    def _enhance_with_feedback(
        self, schema: Dict[str, Any], feedback: List[ExtractionFeedback]
    ) -> None:
        """
        ...
        for i, field in enumerate(schema["fields"]):
            ...
            success_count = sum(
                1 for fb in field_feedback if fb.status == EXTRACTION_SUCCESS
            )
            failure_count = sum(
                1 for fb in field_feedback if fb.status == EXTRACTION_FAILURE
            )
            ...
            partial_feedback = [
                fb for fb in field_feedback if fb.status == EXTRACTION_PARTIAL
            ]
            if partial_feedback:
                self._improve_partial_field(field, partial_feedback)

    def _improve_failing_field(
        self, field: Dict[str, Any], feedback: List[ExtractionFeedback]
    ) -> None:
        """
        ...
        if original_selector:
            ...
            if field.get("attribute") == "text" and field_name.lower() in {
                "price",
                "sale_price",
            }:
            ...
            logger.info(
                f"Enhanced selector for field '{field_name}' from '{original_selector}' to '{field['selector']}'"
            )
    ...
    def _enhance_for_domain(
        self, schema: Dict[str, Any], domain: str, existing_fields: Set[str]
    ) -> None:
        """
        ...
        for field_name, field_config in patterns.items():
            if not any(
                name in existing_fields
                for name in self._get_field_variations(field_name)
            ):
                schema["fields"].append(
                    {
                        ...
                        "description": field_config.get(
                            "description", f"Domain-specific field for {domain}"
                        ),
                    }
                )
                logger.info(
                    f"Added domain-specific field '{field_name}' for '{domain}'"
                )
...
def enhance_schema_from_results(
    ...
    for field_name, value in successful_extractions.items():
        ...
        feedback.append(
            ExtractionFeedback(field_name=field_name, status=status, value=value)
        )

    for field_name, error in failed_extractions.items():
        feedback.append(
            ExtractionFeedback(
                field_name=field_name, status=EXTRACTION_FAILURE, error=str(error)
            )
        )
...
def create_adaptive_schema(
    ...
    if site_patterns:
        existing_fields = {
            f.get("name", "").lower() for f in enhanced.get("fields", [])
        }

        for field_name, pattern in site_patterns.items():
            if not any(
                name in existing_fields
                for name in enhancer._get_field_variations(field_name)
            ):
                enhanced["fields"].append(
                    {
                        ...
                        "description": pattern.get(
                            "description", f"Site-specific field"
                        ),
                    }
                )
