...
from crawl4ai_llm.extraction.schema_validator import (
    SchemaField, SchemaValidator, correct_schema,
    get_schema_improvement_suggestions, validate_schema)
...
class TestSchemaField:
    ...
    def test_valid_schema_field(self):
        ...
        field = SchemaField(
            name="title", selector="h1.product-title", required=True, attribute=None
        )
    ...
    def test_invalid_selector(self):
        ...
        with pytest.raises(SchemaValidationError, match="Invalid selector"):
            SchemaField(
                name="title", selector="", required=True  # Empty selector is invalid
            )
    ...
    def test_optional_field(self):
        ...
        field = SchemaField(
            name="description", selector=".product-description", required=False
        )
    ...
    def test_field_with_attribute(self):
        ...
        field = SchemaField(
            name="image", selector="img.product-image", required=True, attribute="src"
        )
...
class TestSchemaValidator:
    ...
    def test_validate_raises_exception(self):
        ...
        schema = {"title": {"selector": "", "required": True}}
        validator = SchemaValidator(schema)
...
class TestSchemaValidatorIntegration:
    ...
    def test_validate_real_world_schema(self):
        ...
        schema = {
            ...
            "images": {
                "selector": "div.product-gallery img",
                "required": True,
                "attribute": "src",
            },
...
def invalid_schema() -> Dict[str, Any]:
    ...
    return {
        "name": "Invalid Schema",
        "fields": [
            {"name": "title", "attribute": "text"},  # Missing selector
...
class TestSchemaValidator:
    ...
    def test_correct_schema(self, invalid_schema):
        ...
        field_names = {
            field.get("name") for field in corrected["fields"] if "name" in field
        }
    ...
    def test_suggest_improvements(self, valid_schema):
        ...
        problem_schema = {
            "name": "Problem Schema",
            "fields": [
                {"name": "title", "selector": "div", "required": True},  # Too generic
                {
                    "name": "price",
                    "selector": ".product-price",
                    "required": True,
        ...
        assert any(
            "price parsing" in suggestion.lower() for suggestion in suggestions["price"]
        )
