...
class TestContentFilter:
    ...
    def test_str_representation(self):
        ...
        assert str(regex_filter) == "ContentFilter(type=regex, pattern=Price: \\$(\\d+\\.\\d+))"
...
class TestContentFilterIntegration:
    ...
    async def test_base_crawler_with_content_filters(self, mock_crawler):
        ...
        assert result["extracted_content"] == ["<div class='product'>Product Content</div>"]
    ...
    async def test_product_extractor_with_content_filters(self, mock_schema_generator, mock_base_crawler):
...
class TestContentFilterEndToEnd:
    ...
    @pytest.mark.parametrize("filter_type,filter_params,input_html,expected_output", [
        # CSS filter
        (
            "css",
    ...
    async def test_content_filter_application(
        ...
        if filter_type == "css":
            ...
            if content_filter.extract_text:
                ...
                result = etree.tostring(elements[0], encoding='unicode') if elements else ""
                
        else:  # regex
            result = re.sub(content_filter.pattern, content_filter.replacement, input_html)
