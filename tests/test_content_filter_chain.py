...
class TestContentFilterChain:
    ...
    def test_init_validation(self):
        ...
        with pytest.raises(ValueError):
            ContentFilterChain(filters=[ContentFilter()], strategy="INVALID")

        # Valid initialization should not raise errors
        chain = ContentFilterChain(
            filters=[ContentFilter()], strategy="SEQUENCE", name="TestChain"
        )
    ...
    def test_sequence_strategy(self):
        ...
        filter1 = ContentFilter(
            filter_type="css", selector=".product-container", extract_text=False
        )
        filter2 = ContentFilter(
            filter_type="css", selector=".product-title", extract_text=True
        )

        chain = ContentFilterChain(filters=[filter1, filter2], strategy="SEQUENCE")
    ...
    def test_all_strategy(self):
        ...
        filter2 = ContentFilter(filter_type="regex", pattern=r"<h1[^>]*>(.+?)</h1>")

        chain = ContentFilterChain(filters=[filter1, filter2], strategy="ALL")
        ...
        filter3 = ContentFilter(
            filter_type="css", selector=".non-existent", extract_text=True
        )

        chain = ContentFilterChain(filters=[filter1, filter3], strategy="ALL")
    ...
    def test_any_strategy(self):
        ...
        filter2 = ContentFilter(
            filter_type="css", selector=".product-price", extract_text=True
        )

        chain = ContentFilterChain(filters=[filter1, filter2], strategy="ANY")
    ...
    def test_direct_filter_usage(self):
        ...
        try:
            ...
            direct_chain = ContentFilterChain(filters=[css_filter], strategy="ANY")
    ...
    def test_empty_html(self):
        ...
        chain = ContentFilterChain(filters=[filter1], strategy="SEQUENCE")
    ...
    def test_complex_chaining(self):
        ...
        filter1 = ContentFilter(
            filter_type="css", selector=".attribute", extract_text=True
        )
        filter2 = ContentFilter(filter_type="regex", pattern=r"Color: (.+)")
    ...
    def test_string_representation(self):
        ...
        chain = ContentFilterChain(
            filters=[filter1, filter2], strategy="SEQUENCE", name="TestChain"
        )
...
class TestContentFilterChainWithCrawler:
    ...
    async def test_crawler_with_filter_chain(self):
        ...
        chain = ContentFilterChain(
            filters=[filter1, filter2], strategy="ANY", name="ProductInfo"
        )

        # Create a crawler with both individual filters and chains
        css_filter = ContentFilter(
            filter_type="css", selector=".product-description p", extract_text=True
        )

        # Setup mocks for crawl4ai classes
        with patch(
            "crawl4ai_llm.crawler.base.CrawlerRunConfig"
        ) as MockRunConfig, patch(
            "crawl4ai_llm.crawler.base.BrowserConfig"
        ) as MockBrowserConfig, patch(
            "crawl4ai_llm.crawler.base.AsyncWebCrawler"
        ) as MockCrawler:
            ...
            assert any(
                "Test Product" in content for content in result["extracted_content"]
            )
