...
from crawl4ai_llm.storage.base import StorageException, ProductNotFoundError
...
class TestJSONStorage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_product = ProductData(
            ...
            images=[
                ProductImage(url="https://example.com/image1.jpg", alt_text="Product Image 1")
            ],
            brand="Test Brand",
            available=True,
    ...
    def test_save_and_get_product(self):
        ...
        self.assertEqual(retrieved_product.price.current, self.test_product.price.current)
        self.assertEqual(retrieved_product.price.currency, self.test_product.price.currency)
    ...
    def test_uuid_based_storage(self):
        ...
        uuid_config = StorageConfig(
            type="json",
            path=str(self.temp_dir / "uuid_storage"),
            use_uuid=True
        )
        ...
        self.assertRegex(product_id, r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$')
...
def json_storage(temp_storage_dir):
    ...
    config = StorageConfig(
        ...
        filename_template="{product_id}.json"
    )
...
def test_save_product(json_storage, sample_product):
    ...
    expected_path = os.path.join(
        ...
        json_storage.config.filename_template.format(product_id=sample_product.product_id)
    )
