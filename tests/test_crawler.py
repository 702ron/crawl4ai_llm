...
class TestBaseCrawler:
    ...
    async def test_setup_session(self, crawler_config):
        ...
        assert (
            crawler._session._default_headers.get("User-Agent")
            == crawler_config.user_agent
        )
        assert crawler._session._default_headers.get(
            "Accept-Language"
        ) == crawler_config.headers.get("Accept-Language")
    ...
    async def test_get_page_retry_on_error(self, crawler_config):
        ...
        with patch.object(crawler, "_session", MagicMock()) as mock_session:
            mock_session.get = MagicMock(
                side_effect=[asyncio.Future(), asyncio.Future()]
            )
    ...
    async def test_headers_and_cookies(self, crawler_config, mock_response):
        ...
        with patch.object(crawler, "_session", MagicMock()) as mock_session:
            ...
            await crawler.get_page(
                "https://example.com/product", headers=custom_headers, cookies=cookies
            )
    ...
    async def test_context_manager(self, crawler_config):
        ...
        with patch.object(crawler, "setup") as mock_setup, patch.object(
            crawler, "cleanup"
        ) as mock_cleanup:
