...
from crawl4ai_llm.config import (Config, CrawlerConfig, DatabaseConfig,
                                 LLMConfig, StorageConfig)
...
def test_config():
    ...
    return Config(
        ...
        database=DatabaseConfig(url="sqlite:///:memory:", echo=False),
        llm=LLMConfig(
            provider="dummy",
            api_key="test-key",
        ...
        storage=StorageConfig(type="json", path=tempfile.mkdtemp(), use_uuid=True),
    )
...
async def test_full_extraction_flow(test_config, mock_html_content):
    ...
    # Setup mocks
    with patch(
        "crawl4ai_llm.llm.factory.get_llm_client", return_value=MockLLMClient()
    ), patch.object(
        BaseCrawler,
        "get_page",
        return_value=(mock_html_content, "https://example.com/product"),
    ):

        # Initialize components
        crawler = BaseCrawler(config=test_config.crawler)
        extractor = ProductExtractor(
            config=test_config.llm, schema_generator_config=test_config.llm
        )
        ...
        async with crawler:
            ...
            products = storage.list_products(filters={"title": "Test Product"})
...
async def test_error_handling(test_config, mock_html_content):
    ...
    with patch("crawl4ai_llm.llm.factory.get_llm_client", return_value=MockLLMClient()):
        ...
        with patch.object(
            BaseCrawler, "get_page", side_effect=Exception("Connection error")
        ):
            async with crawler:
        ...
        with patch.object(
            ...
            extractor, "extract_with_schema", side_effect=Exception("Schema error")
        ), patch.object(
            extractor, "extract_with_llm", side_effect=Exception("LLM error")
        ):
