...
class TestJSONStorage:
    ...
    async def test_save_product(self, storage, sample_product):
        ...
        with open(file_path, "r") as f:
            ...
            assert content["attributes"]["color"] == sample_product["attributes"]["color"]
    ...
    async def test_product_not_found(self, storage):
        ...
        with pytest.raises(ProductNotFoundError):
            await storage.update_product("non-existent-id", {"title": "Test", "url": "https://example.com"})
    ...
    async def test_complex_filters(self, storage):
        ...
        # Test filtering by brand
        audioproducts, count = await storage.list_products(filters={"store_name": "AudioMax"})
        assert count == 2
    ...
    async def test_concurrent_operations(self, storage):
        ...
        assert all(p["title"].startswith("Concurrent Product") for p in retrieved_products)
    ...
    async def test_advanced_filters(self, storage):
        ...
        # Test filtering by brand
        techpro_products, count = await storage.list_products(filters={"store_name": "TechPro"})
        assert count == 2
        assert all(p["store_name"] == "TechPro" for p in techpro_products)
        
        # Test filtering with partial string match
        laptop_products, count = await storage.list_products(filters={"title": "Laptop"})
        assert count == 2
        assert all("Laptop" in p["title"] for p in laptop_products)
        
        # Test case-insensitive filtering
        tablet_products, count = await storage.list_products(filters={"title": "tablet"})
        assert count == 1
    ...
    async def test_batch_operations_concurrent(self, storage):
        ...
        ids1, ids2 = await asyncio.gather(
            storage.save_products(products1),
            storage.save_products(products2)
        )
        ...
        results1, results2 = await asyncio.gather(
            storage.get_products(ids1),
            storage.get_products(ids2)
        )
        ...
        # Prepare updates
        updates1 = {pid: {"title": f"Updated A{i}", "url": products1[i]["url"], "store_name": "Store A"} 
                   for i, pid in enumerate(ids1)}
        updates2 = {pid: {"title": f"Updated B{i}", "url": products2[i]["url"], "store_name": "Store B"} 
                   for i, pid in enumerate(ids2)}
        
        # Run concurrent batch update operations
        update_results1, update_results2 = await asyncio.gather(
            storage.update_products(updates1),
            storage.update_products(updates2)
        )
        ...
        delete_results1, delete_results2 = await asyncio.gather(
            storage.delete_products(ids1),
            storage.delete_products(ids2)
        )
